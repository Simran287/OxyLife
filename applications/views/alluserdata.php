...
<body>
  <h3>Status for Oxygen Supply</h3>
  <?php
    //echo "<h3>London</h3> <p>London is in England.</p>";
    $alluserdata = $this->session->userdata('AllUserData');
    //echo var_dump($alluserdata);
    foreach($alluserdata as $row)
    {
      ...
      echo "<tr><th>Status</th><td>" . "<form action='' method='post' name='selectstatus'><select name='status' onchange='this.form.submit();'><option value=" . $row->Status . ">" . $row->Status . "</option><option value='Approved'>Approved</option></select></form>" . "</td></tr>";
      echo "</table>";
    }
  ?>
</body>
