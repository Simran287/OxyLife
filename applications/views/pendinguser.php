...
<body>
  <?php
    ...
    $pendinguser = $this->session->userdata('PendingUsers');
    //echo var_dump($pendinguser);
    if(isset($pendinguser))
    {
      ...
      echo "<tr><th>Status</th><td>" . "<form action='' method='POST' name='selectstatus'><select name='status' onchange='this.form.submit();'><option value=" . $row->Status . ">" . $row->Status . "</option><option value='Approved'>Approved</option><option value='Rejected'>Rejected</option></select></form>" . "</td></tr>";
      echo "</table>";
    ...
    {
      echo "There are no Pending Users.";
    }
  ?>
</body>
</html>