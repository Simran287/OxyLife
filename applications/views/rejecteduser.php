...
<body>
  <?php
    ...
    $rejecteduser = $this->session->userdata('RejectedUsers');
    //echo var_dump($rejecteduser);
    if(isset($rejecteduser))
    {
      foreach($rejecteduser as $row)
      {
      ...
      echo "<tr><th>Status</th><td>" . "<form action='' method='post' name='selectstatus'><select name='status' onchange='this.form.submit();'><option value=" . $row->Status . ">" . $row->Status . "</option><option value='Pending'>Pending</option><option value='Approved'>Approved</option></select></form>" . "</td></tr>";
      echo "</table>";
    ...
    {
      echo "There are no Rejected Users.";
    }
  ?>
</body>
</html>